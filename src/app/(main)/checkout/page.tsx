            ...
            {items.map((item) => (
              <div key={item.id} className="flex justify-between items-center text-[16px] font-serif text-[#212529]">
                <div className="flex items-center gap-4">
                  <button type="button" onClick={() => removeItem(item.id)} className="text-red-500 hover:text-red-700 transition-colors font-bold w-6 h-6 flex items-center justify-center cursor-pointer">
                    ✕
                  </button>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center border border-[#CCCCCC] rounded-sm">
                    <button type="button" onClick={() => decreaseQuantity(item.id)} className="px-3 py-1 hover:bg-gray-100">-</button>
                    <span className="px-3 py-1 border-x border-[#CCCCCC]">{item.quantity}</span>
                    <button type="button" onClick={() => increaseQuantity(item.id)} className="px-3 py-1 hover:bg-gray-100">+</button>
                  </div>
        ...
        <div className="space-y-8 bg-gray-50 p-8 rounded-sm border border-[#E5E5E5]">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <input {...register("fullName")} type="text" className="w-full p-4 border border-[#CCCCCC] rounded-sm focus:border-[#AB4227] focus:outline-none transition-colors" />
              {errors.fullName && <p className="text-red-500 text-sm">{errors.fullName.message}</p>}
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Address</label>
              <input {...register("address")} type="text" className="w-full p-4 border border-[#CCCCCC] rounded-sm focus:border-[#AB4227] focus:outline-none transition-colors" />
              {errors.address && <p className="text-red-500 text-sm">{errors.address.message}</p>}
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Phone Number</label>
              <input {...register("phone")} type="tel" className="w-full p-4 border border-[#CCCCCC] rounded-sm focus:border-[#AB4227] focus:outline-none transition-colors" />
              {errors.phone && <p className="text-red-500 text-sm">{errors.phone.message}</p>}
            </div>
            <button disabled={isPending || items.length === 0} className="w-full bg-[#AB4227] text-white py-4 font-serif font-bold uppercase tracking-widest hover:bg-[#8e3620] transition-all rounded-sm cursor-pointer disabled:opacity-50">
