...
export const ChatService = {
  async processZaloEvent(event: any) {
    ...
    if (message.includes('mua ')) {
      ...
      await this.handleOrderPlacement(event.senderId, productName);
  ...
  async handleOrderPlacement(senderId: string, productName: string) {
    ...
    if (productRes.rows.length === 0) {
      ...
      await zaloClient.sendMessage(senderId, `Xin lỗi, mình không tìm thấy sản phẩm '${productName}'.`);
      return;
    }
